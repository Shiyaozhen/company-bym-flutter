...
class BLEManager{
  ...
  Device? connectingDevice; // 我们的SDK是必须有这个属性的，有这个属性才能记录连接的设备，在正式连上之后才会同步getx上的数据，中断时也会同步
...
// MARK:  -  心跳 ♥️♥️♥️♥️♥️♥️♥️♥️♥️♥️♥️♥️♥️♥️♥️♥️♥️♥️♥️♥️♥️♥️♥️♥️♥️♥️♥️♥️♥️♥️♥️♥️♥️♥️♥️♥️♥️♥️♥️♥️♥️♥️♥️♥️♥️♥️♥️♥️♥️♥️♥️♥️♥️♥️♥️♥️♥️♥️♥️♥️♥️♥️♥️♥️♥️♥️♥️♥️
  void startHeart({Duration duration = const Duration(seconds: 2),String? heartCmd}){
    stopHeart();
    heartCmd = heartCmd;
  ...
  void heartHandler() async{
    ...
    if(heartTimeOutCount >= 5){ // 心跳超时
      BYLog.d('心跳超时');
      disConnect();
      
    }else{
      if(stackSend.isEmpty()){ // 当前命令栈为空，发送心跳
  ...
  Future bleConnectDevice(Device device) async{
    ...
    await FlutterBluePlus.adapterState.where(
    ...
    await FlutterBluePlus.isScanning.where((event) => (){
  ...
  Future bleConDevice(BluetoothDevice device) async{
    ...
    await device.connectionState.where((event) => event == BluetoothConnectionState.connected).first;
  ...
  // 发送数据 ################################################
  Future sendCmdAndStartTimet(String cmd,CompleteCallback complete,[int currentCount = 0,Duration duration = const Duration(seconds: 3),int failMaxCount = 5]) async{
  ...
  void receiveHandle(String cmd){
    ...
    BYLog.d('📲📲📲📲📲📲📲📲📲📲📲📲📲📲📲📲📲📲📲接收到命令id:${msgid}content:${cmd}');


    if(!stackSend.has(cmd)){ // 心跳命令没有回调，所以分开
      if(isHeartCmd(cmd)){
        BYLog.d('接收到心跳${cmd}');
        headTimeoutT?.cancel();
        heartTimeOutCount = 0;
      ...
      if(msgid == 4){ // 处理通用命令回调
        ...
        if(connectingDevice!.deviceType != connectingDevice!.chargeGetData.deviceTypeData!.devciceTypeCode){
          BYLog.d('当前型号不准确');
          DeviceTypeData deviceTypeData = connectingDevice!.chargeGetData.deviceTypeData!;
          ...
          // BYDBHealper().updateDevice(connectingDevice!);
        }
  ...
  Future getDeviceType(CompleteCallback complete) async{
    String cmd = BYCmdUtil.makeSendCMD(4, connectingDevice!.pass!.intValue, '',isHeartCmd: false);
    await sendCmdAndStartTimet(cmd,complete);
  }
  
  // 获取控制使能
  Future getControlEnable(CompleteCallback complete) async{
    String cmd = BYCmdUtil.makeSendCMD(113, connectingDevice!.pass!.intValue, '',isHeartCmd: false);
    await sendCmdAndStartTimet(cmd,complete);
  }


  // 设置DLB配置
  Future setDLBMode(bool isDLBEnable,bool isExtremMode,String pvMaxCurrent,bool? autoModeEnable,String? autoModeStartH,String? autoModeEndH,CompleteCallback complete) async{
    ...
    String cmd = BYCmdUtil.makeSendCMD(107, connectingDevice!.pass!.intValue, content,isHeartCmd: false);
    await sendCmdAndStartTimet(cmd,complete);
  }
